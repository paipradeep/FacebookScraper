...
app_id = #add the id generated from fb for developers account
app_secret = #password for the same
page_id = ""
success = "False"
...
def scrapeFacebookPageFeedStatus(page_id, access_token, since_date, until_date):
    with open('{}_facebook_statuses.csv'.format(page_id), 'w') as file:
        ...
        str2 = "\nDone!\n{} Statuses Processed in {}".format(num_processed, datetime.datetime.now() - scrape_starttime)
        #return [str1,str2]
        return [num_processed,datetime.datetime.now() - scrape_starttime]
